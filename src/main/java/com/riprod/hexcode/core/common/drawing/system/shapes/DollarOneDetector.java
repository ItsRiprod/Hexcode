...
public class DollarOneDetector implements ShapeDetector {
    ...
    private static final float ANGLE_RANGE = (float) Math.toRadians(45);
    ...
    private float indicativeAngle(float[][] points) {
        float[] centroid = centroid(points);
        return (float) Math.atan2(centroid[1] - points[0][1], centroid[0] - points[0][0]);
    }
    ...
    private void logTopCandidates(List<String> names, List<Float> scores) {
        ...
        StringBuilder sb = new StringBuilder("[$1] top 3:");
    ...
    public void ensureLoaded() {
        ...
        ShapeAsset.getAssetMap().getAssetMap().forEach((key, asset) -> {
            ...
            if (!templates.isEmpty()) {
                float[][] processed = preprocess(templates.getFirst().getPointsAs2D());

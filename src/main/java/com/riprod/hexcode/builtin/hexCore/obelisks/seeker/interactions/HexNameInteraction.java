package com.riprod.hexcode.builtin.hexCore.obelisks.seeker.interactions;
...
public class HexNameInteraction extends SimpleInteraction {
    private static final HytaleLogger LOGGER = HytaleLogger.forEnclosingClass();

    @Nonnull
    public static final BuilderCodec<HexNameInteraction> CODEC = BuilderCodec
            .builder(HexNameInteraction.class, HexNameInteraction::new, SimpleInteraction.CODEC)
            .build();

    public HexNameInteraction() {
    }
    ...
    protected void tick0(boolean firstRun, float time, @Nonnull InteractionType type,
        ...
        try {
            ...
            if (casterComp == null || !CraftingState.CONTEXT_ID.equals(casterComp.getCurrentContext())) {
                ref.sendMessage(Message.raw("You must be in Crafting Mode to name a hex"));
                ctx.getState().state = InteractionState.Failed;
                return;
            }
            ...
            player.getPageManager().openCustomPage(playerRef, store,
                    new HexNamePage(ref, targetBlockLocation));

            ctx.getState().state = InteractionState.Finished;
            super.tick0(firstRun, time, type, ctx, cooldown);
        } catch (Exception e) {
            LOGGER.atSevere().log("[hexcode] HexNameInteraction failed: %s", e.getMessage());
            ctx.getState().state = InteractionState.Failed;
        }
    }

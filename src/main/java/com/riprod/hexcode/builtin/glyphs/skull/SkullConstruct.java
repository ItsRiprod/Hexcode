...
public class SkullConstruct implements ConstructHandler<SkullState> {

    private static final HytaleLogger LOGGER = HytaleLogger.forEnclosingClass();
    private static final String BURN_EFFECT_ID = "Burn";

    @Override
    public boolean onTick(float dt, HexStatus<SkullState> status, ConstructTickContext ctx) {
        SkullState state = status.getState();
    ...
    @Override
    public void onEnd(HexStatus<SkullState> status, ConstructTickContext ctx) {
        cleanup(ctx);
        SkullState state = status.getState();
    ...
    @Override
    public List<String> getPendingNextGlyphIds(HexStatus<SkullState> status) {
        SkullState state = status.getState();
        return state != null ? state.getNextGlyphIds() : List.of();
    }

    @Override
    public void setPendingNextGlyphIds(HexStatus<SkullState> status, List<String> ids) {
        SkullState state = status.getState();
